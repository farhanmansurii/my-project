  ...
  return (
    <div>
      ...
      {recentlyWatched && recentlyWatched.length > 0 && (
        <>
          <div className="text-2xl text-white w-11/12 mx-auto mt-6 mb-2">
            ...
            <div className="flex flex-nowrap text-white">
              {recentlyWatched.map((e) => (
                <div className="flex flex-col" key={e.tvid}>
                  <div
                    key={e.tvid}
                    className="episode-card flex-none relative w-64 h-36 mb-2 mx-2 rounded max-w-xs"
                  >
                    ...
                    <div
                      style={{
                        ...
                        width: `${Math.round(e.watchTime)}%`,
                      }}
                    ></div>

                    <div className="delete-btn absolute top-2 right-2">
                      <button
                        className="bg-black rounded-full border-white border p-2 hover:scale-110 duration-150"
                        onClick={() => dispatch(deleteEpisode(e.tvid))}
                      >
                        <svg
                          viewBox="0 0 512 512"
                          fill="currentColor"
      ...
      {movies && movies.length > 0 && (
        <div className="overflow-x-auto mx-auto mb-10 w-11/12 ">
          <div className="text-xl text-white mt-6  mb-2">Movies WatchList</div>
          <div className="flex overflow-x-scroll p-2 space-x-4 scrollbar-hide mx-auto ">
            {movies.map((e) => (
              <Link key={e.movieid} href={`/movie/${e.movieid}`}>
                <div className="flex-none w-32 lg:w-40">
                  <div className="relative">
                    ...
                    <div className="absolute flex flex-col-reverse inset-0 p-2 bg-gradient-to-t from-black w-full ">
                      ...
                      <p className="text-xs text-white/40">
                        <span className="text-red-500"> {e.deets.type}</span> •{" "}
...
export default function Home(props) {
  return (
    ...
    >
      <Movies />
      {props.popularMovies && <Rows title={"Popular Movies"} type='movie' data={props.popularMovies} />}
      {props.trendingTV && <Rows title={"Trending TV"} type='tv show' data={props.trendingTV} />}
    </SWRConfig>
  );
}
...
export async function getServerSideProps() {
  ...
  const trendingTVResponse = await axios.get(
    `https://api.themoviedb.org/3/discover/tv?include_adult=false&include_null_first_air_dates=false&language=en-US&page=1&sort_by=primary_release_date.desc&vote_average.lte=10&vote_count.gte=150&with_original_language=en&api_key=${process.env.TMDB_API_KEY}`
  );
  const trendingTV = trendingTVResponse.data.results;
